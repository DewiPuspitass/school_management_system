...
class ExamMarksRegistrationController extends BaseController
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        // Strategi: Lookup Table (Ambil semua jadwal sekaligus untuk efisiensi)
        $schedules = $this->getExamSchedulesLookup($request);

        // Strategi: Guard Clause (Early Return jika jadwal tidak ditemukan)
        ...
        // Strategi: Guard Clause
        if (empty($data)) {
            return $this->returnMessage('You have already Registration Marks or no valid data to insert', 'error');
        }

        $inserted = ExamMarksRegistration::insert($data);

        return $inserted
            ? $this->returnMessage('Registration Marks Inserted Successfully!', 'success')
            : $this->returnMessage('Something went wrong!', 'error');
    }
